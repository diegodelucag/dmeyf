...
palancas$version  <- "v951_0030"   #Muy importante, ir cambiando la version
...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[, cr_mdescubierto_ingresos:= mdescubierto_preacordado/cr_ing_total]
  dataset[, cr_cprod_pasivos:= cr_pasivos/cr_cant_prod ]
  dataset[, cr_cprod_pasivos:= mcaja_ahorro/cpayroll_trx]
  
  dataset[, cr_engagement:= (ctrx_quarter*cr_cant_prod*cr_consumo_tarjeta*
                               cr_ing_total*cr_gastos*mpayroll*cr_desc_pay*
                               mcuentas_saldo*cr_pasivos) ]
...
correr_todo  <- function( palancas )
{
  ...
  dataset  <- fread( "./datasetsOri/datasetsOri_paquete_premium.csv.gz")
