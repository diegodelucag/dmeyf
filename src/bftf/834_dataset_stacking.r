...
version  <- "v0002"  #cambiar cada vez, asi se tiene versionado del dataset

dataset  <- fread( "./datasets/datasets_dataset_epic_v951_0000.csv.gz")
dataset  <- dataset[foto_mes==202009|foto_mes==202010|foto_mes==202011|foto_mes==202101]
dataset  <- copy(  dataset[  , c("numero_de_cliente","foto_mes","clase_ternaria"),  with=FALSE] )
gc()

# fwrite( dataset,
#         file=paste0( "./dataset_orig_sep20_ene21.csv.gz"),
#         sep="," )
...
for( archivo  in archivos )
{
  ...
  dataset  <- merge( dataset, darchivo[foto_mes==202009|foto_mes==202010|foto_mes==202011|foto_mes==202101], by=c("numero_de_cliente","foto_mes") )
}
