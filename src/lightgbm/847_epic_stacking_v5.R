...
kexperimento  <- 1045   #NA si se corre la primera vez, un valor concreto si es para continuar procesando

kscript         <- "847_epic_stacking_v5"

karch_dataset    <- "./datasets/dataset_stacking_v001.csv.gz"   #este dataset se genero en el script 834_dataset_stacking.r
...
hs <- makeParamSet( 
         makeNumericParam("learning_rate",    lower=    0.01 , upper=    0.12),
         makeNumericParam("feature_fraction", lower=    0.1  , upper=    0.9),
         
         #Tamaños de arbol
         makeIntegerParam("min_data_in_leaf", lower=  20L   , upper= 100L), #cantidad de datos por hoja
         makeIntegerParam("num_leaves",       lower=    10L   , upper= 40L), #cantidad de hojas
         makeIntegerParam("max_depth",        lower=    4L   , upper= 10L),    

         #makeNumericParam("min_gain_to_split",lower=    0  , upper=    5.0),
         makeNumericParam("lambda_l1",        lower=    0.01   , upper= 100), 
         makeNumericParam("lambda_l2",        lower=    0.01   , upper= 200),
         makeIntegerParam("max_bin",           lower=    255L   , upper= 1024L),
         makeNumericParam("prob_corte", lower=    0.022  , upper=    0.055),
         
         #Agrego los de bagging
         makeNumericParam("bagging_fraction", lower=    0.2  , upper=    0.9),  
         makeIntegerParam("bagging_freq",        lower=    50L   , upper= 300L)
         )
        
        

campos_malos  <- c("Predicted")   #aqui se deben cargar todos los campos culpables del Data Drifting
...
EstimarGanancia_lightgbm  <- function( x )
{
  ...
  #Bloque de código para buscar max_bin
