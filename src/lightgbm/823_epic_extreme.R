...
#---Cambios----------------------------------------------------------------------
#Agregué hiperparámetros a buscar en BO: max_depth, min_gain_to_split, lambda_l1, lambda_l2
#Modifiqué max_bin 31 por 5 (Zulip)
#Por lo anterior no cargué campos malos
#-------------------------------------------------------------------------------
getwd()
...
kexperimento  <- 103903   #NA si se corre la primera vez, un valor concreto si es para continuar procesando

kscript         <- "823_epic_extreme"
...
hs <- makeParamSet( 
         ...
         makeNumericParam("feature_fraction", lower=    0.4  , upper=    0.99), 
         makeIntegerParam("min_data_in_leaf", lower=  100L   , upper= 4000L), 
         makeIntegerParam("num_leaves",       lower=    20L   , upper= 6000L),  
         makeIntegerParam("max_depth",        lower=    3L   , upper= 12L),    
         makeIntegerParam("lambda_l1",        lower=    0L   , upper= 100L), 
         makeIntegerParam("lambda_l2",        lower=    30L   , upper= 180L),
         makeNumericParam("bagging_fraction", lower=    0.4  , upper=    0.99),  
