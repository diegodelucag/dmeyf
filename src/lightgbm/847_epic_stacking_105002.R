...
kexperimento  <- 105002   #NA si se corre la primera vez, un valor concreto si es para continuar procesando

kscript         <- "847_epic_stacking"

karch_dataset    <- "./datasets/dataset_stacking_modelitos&Stacksv002.csv.gz"   #este dataset se genero en el script 812_dataset_epic.r
...
hs <- makeParamSet( 
         ...
         makeNumericParam("bagging_fraction", lower=    0.3  , upper=    1.0),
         makeIntegerParam("bagging_freq",       lower=    50L, upper= 200L)
...
ctrl  <- makeMBOControl( save.on.disk.at.time= 6000,  save.file.path= kbayesiana)  #se graba cada 600 segundos
